...
`SafeNumber` 是一个轻量级的安全数值计算工具，用于解决 JavaScript 浮点数运算精度问题，并支持货币格式化等扩展能力。

```ts
import { SafeNumber } from 'arcdash'

// 避免浮点误差
SafeNumber(2.51).add(0.01) // 2.52（正确）
SafeNumber(0.1).multiply(0.2) // 0.02（正确）

// 格式化
SafeNumber(1000).format() // "¥1,000.00"

// 平均分配
SafeNumber(10).distribute(3) // [3.34, 3.33, 3.33]
```
