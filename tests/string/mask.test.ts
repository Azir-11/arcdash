...
describe("mask function", () => {
  ...
  describe("minMaskLen（最小密文位数）", () => {
    ...
    it("minMaskLen 大于实际掩码位数时增加掩码", () => {
      expect(_.mask("abcdefghij", { prefixLen: 4, suffixLen: 4, minMaskLen: 4 })).toBe(
        "abc****hij",
      );
    });

    it("minMaskLen 与 position: front 配合", () => {
      expect(_.mask("12345678", { position: "front", suffixLen: 2, minMaskLen: 4 })).toBe(
        "******78",
      );
    });

    it("minMaskLen 与 position: back 配合", () => {
      expect(_.mask("12345678", { position: "back", prefixLen: 2, minMaskLen: 4 })).toBe(
        "12******",
      );
    });
  ...
  describe("minVisibleLen（最少保留明文位数）", () => {
    describe("position: middle", () => {
      ...
      it("minVisibleLen=2 时保留至少 2 位明文", () => {
        expect(_.mask("12345678", { prefixLen: 0, suffixLen: 0, minVisibleLen: 2 })).toBe(
          "******78",
        );
      });

      it("明文已经满足 minVisibleLen 时不变", () => {
        expect(_.mask("12345678", { prefixLen: 2, suffixLen: 2, minVisibleLen: 1 })).toBe(
          "12****78",
        );
      });
    });
    ...
    describe("position: front", () => {
      it("minVisibleLen=3 时保留至少 3 位尾部明文", () => {
        expect(_.mask("12345678", { position: "front", suffixLen: 0, minVisibleLen: 3 })).toBe(
          "*****678",
        );
      });

      it("suffixLen 已满足 minVisibleLen 时不变", () => {
        expect(_.mask("12345678", { position: "front", suffixLen: 4, minVisibleLen: 2 })).toBe(
          "****5678",
        );
      });
    });
    ...
    describe("position: back", () => {
      it("minVisibleLen=3 时保留至少 3 位前部明文", () => {
        expect(_.mask("12345678", { position: "back", prefixLen: 0, minVisibleLen: 3 })).toBe(
          "123*****",
        );
      });

      it("prefixLen 已满足 minVisibleLen 时不变", () => {
        expect(_.mask("12345678", { position: "back", prefixLen: 4, minVisibleLen: 2 })).toBe(
          "1234****",
        );
      });
    });
    ...
    describe("与 minMaskLen 交互", () => {
      it("minMaskLen=3, minVisibleLen=1，字符串足够长", () => {
        expect(
          _.mask("abcdefgh", { prefixLen: 2, suffixLen: 2, minMaskLen: 3, minVisibleLen: 1 }),
        ).toBe("ab****gh");
      });

      it("minMaskLen=3, minVisibleLen=0", () => {
        expect(
          _.mask("abcdefgh", { prefixLen: 2, suffixLen: 2, minMaskLen: 3, minVisibleLen: 0 }),
        ).toBe("ab****gh");
      });
      ...
      it("minMaskLen 优先级高于 minVisibleLen", () => {
        expect(
          _.mask("abcdef", { prefixLen: 2, suffixLen: 2, minMaskLen: 4, minVisibleLen: 4 }),
        ).toBe("a****f");
      });
    });
    ...
    describe("实际场景", () => {
      ...
      it("姓名脱敏：张三，minVisibleLen=0", () => {
        expect(
          _.mask("张三", { minMaskLen: 3, prefixLen: 1, suffixLen: 0, minVisibleLen: 0 }),
        ).toBe("**");
      });
  ...
  describe("长字符串", () => {
    it("身份证号码脱敏", () => {
      expect(_.mask("110101199003074519", { prefixLen: 3, suffixLen: 4 })).toBe(
        "110***********4519",
      );
    });

    it("银行卡号脱敏", () => {
      expect(_.mask("6222021234567890123", { prefixLen: 4, suffixLen: 4 })).toBe(
        "6222***********0123",
      );
    });
